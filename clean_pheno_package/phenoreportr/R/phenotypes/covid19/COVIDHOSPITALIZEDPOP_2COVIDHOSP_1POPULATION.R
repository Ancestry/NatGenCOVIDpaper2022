...
clean_phenotype = function(survey) {
  ...
  hosp_covid = hosp_covid %>% 
    ...
    dplyr::mutate(COVIDHOSPITALIZEDPOP_2COVIDHOSP_1POPULATION=ifelse(response1=="Yes,andwaspositive" & response2=="Yes", 2, 1)) %>%
    dplyr::select(fid, iid, question, response, related, split, platform, COVIDHOSPITALIZEDPOP_2COVIDHOSP_1POPULATION) %>%
