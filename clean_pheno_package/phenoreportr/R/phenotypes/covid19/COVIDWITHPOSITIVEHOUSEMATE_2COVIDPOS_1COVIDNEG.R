...
clean_phenotype = function(survey) {
  ...
  house_covid = house_covid %>% 
    ...
    dplyr::mutate(COVIDWITHPOSITIVEHOUSEMATE_2COVIDPOS_1COVIDNEG=ifelse(response1=="Yes,andwaspositive" & response2=="Yes,atleastoneperson", 2,
                                                                 ifelse(response1=="Yes,andwasnegative" & response2=="Yes,atleastoneperson", 1, -9))) %>%
    dplyr::select(fid, iid, question, response, related, split, platform, COVIDWITHPOSITIVEHOUSEMATE_2COVIDPOS_1COVIDNEG) %>%
